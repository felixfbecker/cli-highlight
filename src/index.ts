...
import { Text, Node, Element, Document } from 'domhandler'

function colorizeNode(node: Node, theme: Theme = {}, context?: string): string {
    switch (node.type) {
        case 'text': {
        ...
        case 'tag': {
            const hljsClass = /hljs-(\w+)/.exec((node as Element).attribs.class)
            if (hljsClass) {
                const token = hljsClass[1]
                const nodeData = (node as Element).childNodes
            ...
            return (node as Element).childNodes.map(node => colorizeNode(node, theme)).join('')
        }
    }
...
function colorize(code: string, theme: Theme = {}): string {
    const fragment = parse5.parseFragment(code, {
        treeAdapter,
    }) as Document // HtmlParser2.DocumentFragment
